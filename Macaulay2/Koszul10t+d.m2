...
ofilename="result_Koszul"|D|"t+d2_5.txt"
...
for n from 2 to sub(D/2,ZZ) do	--could change for loop to parallel apply or scan on a function--
(
			M = HH_n K;
...
----			for igen from 0 to min(2,(numgens resM_j)-1) do
----			(
				dideltaa=sub(gens resM_j,S);	--consider one generator, a_i=d^-1 delta a_i+1--
					--DEBUG--
				...
				(
					...
					dideltaa=matrix((entries sum(1..Nspin,i'->p_i'*diff(t_i',transpose deltaa)))/(j'->((toList j')/(k'->(if sum(degree(k'+1)) ==0 then 0 else k'/sum(degree(k'+1)) )))));
					--dideltaa=sum(1..Nspin,i'->p_i'*diff(t_i',matrix((entries transpose deltaa)/(j'->((toList j')/(k'->(if sum(degree(k'+1)) ==0 then 0 else k'/sum(degree(k'+1)) )))))));
					...
					sort unique apply(0..(numrows(dideltaa)-1), i'->if dideltaa^{i'}==0 then reachzero=sort unique(reachzero|{{n,j,sum ((degrees (gens resM_j))_0)_i',select(sort unique ((flatten entries lastdideltaa^{i'})/(j'->degree(j'))),k'->#k'>=2)}}));--parallel apply or scan (no output)
					--select(sort unique ((flatten entries lastdideltaa_{i'})/(j'->degree(j'))),k'->#k'>=2);	--select two degrees, and remove infinity
					idelta=idelta-1;
				);
				if dideltaa==0 then
				(
